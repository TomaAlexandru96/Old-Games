...
public class MainOnline : Photon.MonoBehaviour {
	private int Xstart1;
	private int Ystart1;
	private int Xstart2;
	private int Ystart2;
	...
	private string pion = "pion";
	private string tura = "tura";
	private string cal = "cal";
	private string nebun = "nebun";
	private string regina = "regina";
	private string rege = "Test2";
	private string pionN = "pionNegru";
	private string turaN = "turaNegru";
	private string calN = "calNegru";
	private string nebunN = "nebunNegru";
	private string reginaN = "reginaNegru";
	private string regeN = "RegeNV2";
	...
	int move(int x, int y, int x1, int y1){
		...
		if (player == 1){
					...
					if (Chess(6) == 0){
						...
						int viewDeleteId = GameObject.Find (x1 + " " + y1).GetComponent<PhotonView>().viewID;

							//photonView.RPC ("destroyOverNetwork", PhotonTargets.MasterClient,viewDeleteId);
							ok=1;	
							if (aux == -1){
								ktPB++;
							}
				...
				//photonView.RPC ("destroyOverNetwork", PhotonTargets.MasterClient,GameObject.Find (x1 + " " + y1));
					}
					else{
						a[x][y] = a[x1][y1];
						a[x1][y1] = aux;
		...
		if (player == 2){
			...
			//	photonView.RPC ("destroyOverNetwork", PhotonTargets.Others,GameObject.Find (x1 + " " + y1));
					}
					else{
						a[x][y] = a[x1][y1];
						a[x1][y1] = aux;
			...
			for (i=1; i <= 8; i++) {
				for (j=1; j <= 8; j++) {
					if (a [i] [j] == 1) {
						...
						data[0] = i + " " + j;
						//Debug.Log (data[0]);
						clone = PhotonNetwork.InstantiateSceneObject (pion, g.transform.position, g.transform.rotation, 0,data) as GameObject;
						clone.transform.name = i + " " + j;
							//clone.GetComponent<PhotonView> ().viewID = 1;
					...
					if (a [i] [j] == 2) {
						...
						g = GameObject.Find (formare);
						data[0] = i + " " + j;
						clone = PhotonNetwork.InstantiateSceneObject (tura, g.transform.position, g.transform.rotation, 0,data) as GameObject;
						clone.transform.name = i + " " + j;
						//clone.GetComponent<PhotonView> ().viewID = 1;
						//Debug.Log(clone.transform.name);
					}
					if (a [i] [j] == 3) {
						...
						g = GameObject.Find (formare);
						data[0] = i + " " + j;
						clone = PhotonNetwork.InstantiateSceneObject (cal, g.transform.position, g.transform.rotation, 0,data) as GameObject;
						clone.transform.name = i + " " + j;
						//clone.GetComponent<PhotonView> ().viewID = 1;
						//Debug.Log(clone.transform.name);
					}
					if (a [i] [j] == 4) {
						...
						g = GameObject.Find (formare);
						data[0] = i + " " + j;
						clone = PhotonNetwork.InstantiateSceneObject (nebun, g.transform.position, g.transform.rotation, 0,data) as GameObject;
						clone.transform.name = i + " " + j;
						//clone.GetComponent<PhotonView> ().viewID = 1;
						//Debug.Log(clone.transform.name);
					}
					if (a [i] [j] == 5) {
						...
						g = GameObject.Find (formare);
						data[0] = i + " " + j;
						clone = PhotonNetwork.InstantiateSceneObject (regina, g.transform.position, g.transform.rotation, 0,data) as GameObject;
						clone.transform.name = i + " " + j;
						//Debug.Log(clone.transform.name);
						//clone.GetComponent<PhotonView> ().viewID = 1;
					}
					if (a [i] [j] == 6) {
						...
						g = GameObject.Find (formare);
						data[0] = i + " " + j;
						clone = PhotonNetwork.InstantiateSceneObject (rege, g.transform.position, g.transform.rotation, 0,data) as GameObject;
						clone.transform.name = i + " " + j;
						//Debug.Log(clone.transform.name);
						//clone.GetComponent<PhotonView> ().viewID = 1;
					}
					...
					if (a [i] [j] == -1) {
						...
						g = GameObject.Find (formare);
						data[0] = i + " " + j;
						clone = PhotonNetwork.InstantiateSceneObject (pionN, g.transform.position, g.transform.rotation, 0,data) as GameObject;
						clone.transform.name = i + " " + j;
						//	Debug.Log(clone.transform.name);
						//clone.GetComponent<PhotonView> ().viewID = 1;
					}
					if (a [i] [j] == -2) {
						...
						g = GameObject.Find (formare);
						data[0] = i + " " + j;
						clone = PhotonNetwork.InstantiateSceneObject (turaN, g.transform.position, g.transform.rotation, 0,data) as GameObject;
						clone.transform.name = i + " " + j;
						//	Debug.Log(clone.transform.name);
						//clone.GetComponent<PhotonView> ().viewID = 1;
					}
					if (a [i] [j] == -3) {
						...
						g = GameObject.Find (formare);
						data[0] = i + " " + j;
						clone = PhotonNetwork.InstantiateSceneObject (calN, g.transform.position, g.transform.rotation, 0,data) as GameObject;
						clone.transform.name = i + " " + j;
						//	Debug.Log(clone.transform.name);
						//clone.GetComponent<PhotonView> ().viewID = 1;
					}
					if (a [i] [j] == -4) {
						...
						g = GameObject.Find (formare);
						data[0] = i + " " + j;
						clone = PhotonNetwork.InstantiateSceneObject (nebunN, g.transform.position, g.transform.rotation, 0,data) as GameObject;
						clone.transform.name = i + " " + j;
						//clone.GetComponent<PhotonView> ().viewID = 1;
						//Debug.Log(clone.transform.name);
					}
					if (a [i] [j] == -5) {
						...
						g = GameObject.Find (formare);
						data[0] = i + " " + j;
						clone = PhotonNetwork.InstantiateSceneObject (reginaN, g.transform.position, g.transform.rotation, 0,data) as GameObject;
						clone.transform.name = i + " " + j;
						//clone.GetComponent<PhotonView> ().viewID = 1;
						//Debug.Log(clone.transform.name);
					}
					if (a [i] [j] == -6) {
						...
						g = GameObject.Find (formare);
						data[0] = i + " " + j;
						clone = PhotonNetwork.InstantiateSceneObject (regeN, g.transform.position, g.transform.rotation, 0,data) as GameObject;
						clone.transform.name = i + " " + j;
						//clone.GetComponent<PhotonView> ().viewID = 1;
						//	Debug.Log(clone.transform.name);
					}
	...
	{		
		...
		if ((int)data [3] == 1) {
			//GameObject.Find (data[4] + " " + ym).transform.position = Vector3.Lerp (GameObject.Find (xm + " " + ym).transform.position, GameObject.Find ("p" + xm1 + ym1).transform.position, speed);
			//Debug.Log ("xS:"+ (string) data [4] + " yS:" + (string)  data [5] + " xF:" + (string) data [6] + " yF:" + (string) data [7]);


			string nustiucedrqdenumesamaipun;
			string nustiucedrqdenumesamaipun2;
			nustiucedrqdenumesamaipun = data[4].ToString() + " " + data[5].ToString();
			nustiucedrqdenumesamaipun2 =  "p"+data[6].ToString()  +  data[7].ToString();

			//Debug.Log (nustiucedrqdenumesamaipun+ " " + nustiucedrqdenumesamaipun2);

			StartCoroutine(Motion(GameObject.Find (nustiucedrqdenumesamaipun),GameObject.Find (nustiucedrqdenumesamaipun2).transform.position));

			GameObject obiectuldhsvhs = GameObject.Find (nustiucedrqdenumesamaipun);

			obiectuldhsvhs.gameObject.transform.name = data[6].ToString() + " " +  data[7].ToString();
		}
...
//		Debug.Log(afisare);
	...
	void Update () {
		...
		if (PhotonNetwork.room.playerCount == 2 && execOdata == 1) {
			//Debug.Log("PhotonNetwork.room.playerCount == 2 && execOdata == 1");
			if(!PhotonNetwork.isMasterClient){
				GameObject[] piesele = GameObject.FindGameObjectsWithTag("Piesa");
				foreach (GameObject obiectul in piesele){
					object[] oldData = new object[5];
					oldData = obiectul.GetComponent<PhotonView>().instantiationData;
					

					
					obiectul.name = oldData[0].ToString();
				}
			}
			execOdata = 0;
		}
			...
			if (alege == 1) {
				...
				if (moveP == 0) {
					if (player == 2) {
						if (piesaInlocuita == 2) {
							formare = xP + " " + yP;
							PhotonNetwork.Destroy (GameObject.Find (formare));
							data[0] = formare;
							clone = PhotonNetwork.InstantiateSceneObject (tura, GameObject.Find ("p" + xP + yP).transform.position, GameObject.Find ("p" + xP + yP).transform.rotation, 0,data) as GameObject;
							//clone.GetComponent<PhotonView> ().viewID = 1;
							clone.name = formare;
							formare = "";
							alege = 0;
						...
						if (piesaInlocuita == 3) {
							...
							data[0] = formare;
							PhotonNetwork.Destroy (GameObject.Find (formare));
							clone = PhotonNetwork.InstantiateSceneObject (cal, GameObject.Find ("p" + xP + yP).transform.position, GameObject.Find ("p" + xP + yP).transform.rotation, 0,data) as GameObject;
							//clone.GetComponent<PhotonView> ().viewID = 1;
							clone.name = formare;
							formare = "";
							alege = 0;
						...
						if (piesaInlocuita == 4) {
							...
							data[0] = formare;
							PhotonNetwork.Destroy (GameObject.Find (formare));
							clone = PhotonNetwork.InstantiateSceneObject (nebun, GameObject.Find ("p" + xP + yP).transform.position, GameObject.Find ("p" + xP + yP).transform.rotation, 0,data) as GameObject;
							//clone.GetComponent<PhotonView> ().viewID = 1;
							clone.name = formare;
							formare = "";
							alege = 0;
						...
						if (piesaInlocuita == 5) {
							...
							data[0] = formare;
							PhotonNetwork.Destroy (GameObject.Find (formare));
							clone = PhotonNetwork.InstantiateSceneObject (regina, GameObject.Find ("p" + xP + yP).transform.position, GameObject.Find ("p" + xP + yP).transform.rotation, 0,data) as GameObject;
							//clone.GetComponent<PhotonView> ().viewID = 1;
							clone.name = formare;
							formare = "";
							alege = 0;
						...
						if (piesaInlocuita == 2) {
							...
							data[0] = formare;
							PhotonNetwork.Destroy (GameObject.Find (formare));
							clone = PhotonNetwork.InstantiateSceneObject (turaN, GameObject.Find ("p" + xP + yP).transform.position, GameObject.Find ("p" + xP + yP).transform.rotation, 0,data) as GameObject;
							//clone.GetComponent<PhotonView> ().viewID = 1;
							clone.name = formare;
							formare = "";
							alege = 0;
						...
						if (piesaInlocuita == 3) {
							...
							data[0] = formare;
							PhotonNetwork.Destroy (GameObject.Find (formare));
							clone = PhotonNetwork.InstantiateSceneObject (calN, GameObject.Find ("p" + xP + yP).transform.position, GameObject.Find ("p" + xP + yP).transform.rotation, 0,data) as GameObject;
							//clone.GetComponent<PhotonView> ().viewID = 1;
							clone.name = formare;
							formare = "";
							alege = 0;
						...
						if (piesaInlocuita == 4) {
							...
							data[0] = formare;
							PhotonNetwork.Destroy (GameObject.Find (formare));
							clone = PhotonNetwork.InstantiateSceneObject (nebunN, GameObject.Find ("p" + xP + yP).transform.position, GameObject.Find ("p" + xP + yP).transform.rotation, 0,data) as GameObject;
							//clone.GetComponent<PhotonView> ().viewID = 1;
							clone.name = formare;
							formare = "";
							alege = 0;
						...
						if (piesaInlocuita == 5) {
							...
							data[0] = formare;
							PhotonNetwork.Destroy (GameObject.Find (formare));
							clone = PhotonNetwork.InstantiateSceneObject (reginaN, GameObject.Find ("p" + xP + yP).transform.position, GameObject.Find ("p" + xP + yP).transform.rotation, 0,data) as GameObject;
							//clone.GetComponent<PhotonView> ().viewID = 1;
							clone.name = formare;
							formare = "";
							alege = 0;
				...
				if (Physics.Raycast (camRay, out floorHit, camRayLength, floorMask)) {
					...
					if (isMoving == 1) {
						...
						Yfin1 = ym1;
								...
								//StartCoroutine(Motion(GameObject.Find (xm + " " + ym),GameObject.Find ("p" + xm1 + ym1).transform.position));
							...
							if (auxiliar == 81 && mokk == 0) {
								GameObject.Find ("8 1").transform.position = Vector3.Lerp (GameObject.Find ("8 1").transform.position, GameObject.Find ("p84").transform.position, speed);
									...
									//StartCoroutine(Motion(GameObject.Find ("8 1"),GameObject.Find ("p84").transform.position));
							...
							if (auxiliar == 88 && mokk == 0) {
								...
								GameObject.Find ("8 8").transform.position = Vector3.Lerp (GameObject.Find ("8 8").transform.position, GameObject.Find ("p86").transform.position, speed);
									//StartCoroutine(Motion(GameObject.Find ("8 8"),GameObject.Find ("p86").transform.position));
							...
							if (auxiliar == 11 && mokk == 0) {
								GameObject.Find ("1 1").transform.position = Vector3.Lerp (GameObject.Find ("1 1").transform.position, GameObject.Find ("p14").transform.position, speed);
									//StartCoroutine(Motion(GameObject.Find ("1 1"),GameObject.Find ("p14").transform.position));
							...
							if (auxiliar == 18 && mokk == 0) {
								...
								GameObject.Find ("1 8").transform.position = Vector3.Lerp (GameObject.Find ("1 8").transform.position, GameObject.Find ("p16").transform.position, speed);
									//StartCoroutine(Motion(GameObject.Find ("1 8"),GameObject.Find ("p16").transform.position));
						...
						object[] dataToBeSent = new object[10];
						dataToBeSent[0] = player;
						dataToBeSent[1] = a;
						dataToBeSent[2] = b;
						dataToBeSent[3] = caz;
						dataToBeSent[4] = Xstart1;
						dataToBeSent[5] = Ystart1;
						dataToBeSent[6] = Xfin1;
						dataToBeSent[7] = Yfin1;

						photonView.RPC ("rpcFunction", PhotonTargets.Others,dataToBeSent);
	...
	IEnumerator Motion(GameObject obj,Vector3 position)
	{
		float t = 0;
		while (t < 10) {
			t += Time.deltaTime;
			obj.transform.position = Vector3.Lerp (obj.transform.position, position, speed);
			yield return null;
		}
	}
	[PunRPC] void destroyOverNetwork( int id){
